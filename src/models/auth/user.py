...
class User(Base):
    ...
    phone_number_verified: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)  # 전화번호 인증 여부

    # Cognito
    cognito_id: Mapped[str] = mapped_column(String(64), nullable=False, unique=True, index=True)  # Cognito에서 발급하는 고유 ID(UUID)

    given_name: Mapped[str] = mapped_column(String(50), nullable=False)     # 이름
    family_name: Mapped[str] = mapped_column(String(50), nullable=False)    # 성
